...
# NAMEOBJECT = 'coconut_te0a_box'
# NAMEOBJECT = 'calcium_antacid'
# NAMEOBJECT = "monster_truck"
NAMEOBJECT = "ogx_shampoo"
...
class grasper():
    def __init__(self, nameObject=''):
        ...
        self.rp.log_message(
            'Please run the following command in a new terminal (in intera mode) on the kinectbox02 (ssh):')
    ...
    def obj_func(self, x):
        ...
        self.limb.set_joint_position_speed(0.12)
    ...
    def reset_gripper(self, x):
        ...
        self.gripper.open(speed=80)  # Open gripper
        time.sleep(0.5)
        # self.goto_EE_xyz(xyz=des_EE_xyz_above, orientation=des_orientation)
    ...
    def attempt_lift_(self, x):
        ...
        self.goto_EE_xyz(xyz=des_xyz + np.array([0, 0, 0.02]), orientation=des_orientation)
    ...
    def grasp_object(self, force):
        ...
        self.gripper.graspmove_nopending(width=5, speed=80)

        time.sleep(2)

        gripper_force = self.gripper.get_force()
    ...
    def start_experiment(self):
        ...
        data_recorder = DataRecorder(limb=self.limb, gripper=self.gripper, GelSightA=self.gelSightA,
                                     GelSightB=self.gelSightB,
                                     KinectA=self.kinectA,
                                     KinectB=self.kinectB)

        # Initialize recording to file
        nameFile = time.strftime("%Y-%m-%d_%H%M%S") + "_randomized_search_2"
        frequency = 30
        thread = threading.Thread(target=data_recorder.init_record, args=(nameFile, frequency))
        thread.start()
        time.sleep(0.2)
        ...
        path = "/home/manu/ros_ws/src/manu_research/data/"
