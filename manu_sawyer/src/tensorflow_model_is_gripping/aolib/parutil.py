...
class Parallel:
  ...
  def reload_modules(self):
    self.execute('from aolib import parutil as parutil, areload; parutil.run_reload()')
...
def test_partition():
  def f(x):
    import parutil
    return sum(parutil.partition(2, 1).map_sync(lambda x: x, range(10)))
  import parutil
  parutil.map_sync(lambda x:x, range(10000))
  assert sum(parutil.partition(2, 0).map_sync(f, range(1000))) == 1000*sum(range(10))
