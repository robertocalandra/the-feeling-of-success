...
# COMPUTE_BG = True  # Store a new background image
save_parallel = False
bounds_table = np.array([[0.45, 0.65], [-0.25, 0.25]])  # X(min, max), Y(min, max) # TODO: this is too small!!!
grasping_force = [5, 35]  # min, max of the force [N] applied by the gripper when trying to grasp the object # max = 25
...
# name = "fake_plastic_transformer_toy"
# name = "mentos_gum_can"
# name = "fake_flower_in_pot"
# name = "black_beans_bag"
# name = "shower_gel_japanese_blossom"  # sucks
# name = "creamy_petroleum_jelly"
# name = "blue_bottle_fuel_treatment"
# name = "happy_fall_stone"
# name = "fox_head"
# name = "peanut_butter"
# name = "soft_toy_dragon"
# name = "soft_beer_bottle_holder"
...
def init_robot(limb_name):
    ...
    limb.set_joint_position_speed(0.35)
...
def goto_EE_xyz(limb, xyz, orientation=Orientations.DOWNWARD_ROTATED, verbosity=1, rest_pos=False):
    ...
    try:
        ...
        limb.move_to_joint_positions(joint_positions, timeout=2, threshold=0.01)  # Send the command to the arm
        # print("done moving")
...
def grasp_object(gripper, data_recorder):
    ...
    gripper.graspmove_nopending(width=5, speed=80)
    time.sleep(0.1)
    gripper.graspmove_nopending(width=5, speed=80)
    time.sleep(0.1)
    gripper.graspmove_nopending(width=5, speed=80)
...
def main():
    ...
    while run:
        ...
        time.sleep(1.0)  # This is crucial!!!! keep it!
        ...
        if is_gripping:
            ...
            gripper.open(speed=300)  # Open gripper
            time.sleep(0.5)
            limb.set_joint_position_speed(0.30)  # We can move faster

            # Go up, but a little higher than before
            des_EE_xyz_above[2] = 0.60
            goto_EE_xyz(limb=limb, xyz=des_EE_xyz_above, orientation=random_orientation)
            limb.set_joint_position_speed(0.35)  # We can move faster
