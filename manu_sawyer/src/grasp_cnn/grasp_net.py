...
def net_feats(ims, num_classes = None, is_training = True, reuse = False, scope = ''):
  ...
  if net_type == 'resnet':
    feats = resnet_v1.resnet_v1_50(ims, num_classes = num_classes, 
                                   reuse = reuse, is_training = is_training, 
                                   scope = '%s_resnet_v1_50' % scope)[0]
    feats = slim.flatten(feats)
  elif net_type == 'vgg':
    feats = vgg.vgg_16(ims, num_classes = None, scope = '%s_vgg_16' % scope, 
                       reuse = reuse, is_training = is_training)[0]
...
def net_init_path():
  if net_type == 'resnet':
    return '../results/resnet_v1_50.ckpt'
  elif net_type == 'vgg':
    return '../results/vgg_16.ckpt'
...
#def write_data(out_dir, rebalance_data = True, train_frac = 0.75, val_frac = 0.0, n = 10):
  ...
  all_db_files = map(os.path.abspath, ut.read_lines(pj(out_dir, 'all_db_files.txt'))[:n])
  ...
  with slim.arg_scope(net_arg_scope()):
    ...
    if 'gel' in pr.inputs:
      ...
      if gel0_pre is not None:
        feats.append(net_feats(gel0_post - gel0_pre, scope = 'gel', 
                                  is_training = train, reuse = reuse))
        gel_reuse = True
      if gel1_pre is not None:
        feats.append(net_feats(gel1_post - gel1_pre, scope = 'gel', 
                               is_training = train, reuse = gel_reuse))
        gel_reuse = False

    if 'im' in pr.inputs:
      feats.append(net_feats(n(inputs['im0_pre']), num_classes = None, 
                                scope = 'im', is_training = train, reuse = reuse))
      feats.append(net_feats(n(inputs['im0_post']), num_classes = None, 
                                scope = 'im', reuse = True, is_training = train))
    print map(shape, feats)

    if 'depth' in pr.inputs:
      feats.append(net_feats(d(inputs['depth0_pre']), num_classes = None, 
                                scope = 'depth', is_training = train, reuse = reuse))
      feats.append(net_feats(d(inputs['depth0_post']), num_classes = None, 
                                scope = 'depth', is_training = train, reuse = True))

    print map(shape, feats)
    if 'ee' in pr.inputs:
      net = inputs['ee']
      net = slim.batch_norm(net, scope = 'ee/bn', renorm = True)
      #net = net / ed(tf.sqrt(tf.reduce_sum(net**2, 1)), 1)
      net = slim.fully_connected(net, 2048, scope = 'ee/fc1', reuse = reuse)
      net = slim.fully_connected(net, 2048, scope = 'ee/fc2', reuse = reuse)
      net = slim.fully_connected(net, 2048, scope = 'ee/fc3', reuse = reuse)
      feats.append(net)
    print map(shape, feats)
    net = tf.concat(feats, 1)
    if pr.cls_style == 'mlp-1':
      net = slim.fully_connected(net, 4096, scope = 'merged/fc1', reuse = reuse)
      net = slim.dropout(net, is_training = train)
      logits = slim.fully_connected(net, 2, scope = 'logits', activation_fn = None, 
                                    reuse = reuse)
    elif pr.cls_style == 'linear':
      logits = slim.fully_connected(net, 2, scope = 'logits', 
...
def train(pr, gpus, restore = False, use_reg = True):
  ...
  with tf.Graph().as_default(), tf.device(gpus[0]), tf.Session(config = config) as sess:
    ...
    for i in ut.time_est(range(pr.train_iters)):
      ...
      _, lr_val, cls_loss_val, loss_val, acc_val = sess.run([train_op, lr, cls_loss, loss, acc])
...
# def pix_from_ee(pt1):
#   # (H, t) transforms a point in the Kinect coordinate system to the sawyer.
#   # It comes from calibrate.py, and corresponds to the nordri robot
#   H = np.array([[ -2.52227094e-02,   1.89235532e-01,  -1.07166317e+00],
#                 [ -9.72831462e-01,   1.45903784e+00,   1.03496641e+00],
#                 [  6.34583087e-04,  -4.07683700e-01,  -2.86833642e-01]])
#   t = np.array([1.47331984, -0.84012246, 0.22456334])
#   # Apply the inverse transform
#   # y = H*x + t
#   # x = solve(H, y - t)
#   pt2 = np.linalg.solve(H, pt1 - t)
#   proj = ut.inhomog(camera_matrix().dot(pt2))
#   return proj


# def crop_ee(im, ee):
#   # ee = location_of_EE_at_grasping
#   proj = map(int, pix_from_kinect(ee))
#   # for debugging
#   im = ig.draw_pts(im, [proj], width = 4)
#   dim = int(0.2*(0.5*(im.shape[0] + im.shape[1])))
#   rect = ut.rect_centered_at(proj[0], proj[1], dim, dim)
#   return ig.sub_img_pad(im, rect)
...
def crop_obj(im, center):
  # ee = location_of_EE_at_grasping
  proj = map(int, pix_from_kinect(center))
  # for debugging
  im = ig.draw_pts(im, [proj], width = 4)
  dim = int(0.5*(0.5*(im.shape[0] + im.shape[1])))
  rect = ut.rect_centered_at(proj[0], proj[1], dim, dim)
  return ig.sub_img_pad(im, rect)
...
def vis_example(db_file):
  with h5py.File(db_file, 'r') as db:
    ...
    im_crop = sc(crop_obj(im_base, np.array(db['cylinder_data']['xyz_kinect'].value)))
...
def eval_splits(pr_fn, n, gpu):
  accs = []
  for i in xrange(n):
    pr = pr_fn(i)
    accs.append(test(pr, gpu))
  print ut.mean_pm_str(np.mean(accs), n)
  return np.mean(accs)

def eval_models(pr_fns, n, gpu):
  accs = [eval_splits(pr, n, gpu) for pr in pr_fns]
  for pr, acc in zip(pr_fns, accs):
    print pr(0).description, '&', 100.*acc, '\\\\'

# def show_examples(pr):
#   data = ut.load(pj(pr.dsdir, 'train.pk'))  
#   for d in data:
#     d['
